...
class clustering_metrics():
    ...
    def evaluationClusterModelFromLabel(self, a, b, c):
        ...
        fh.write(
            'GAAMA=%f,k=%f,a=%f,ACC=%f, f1_macro=%f, precision_macro=%f, recall_macro=%f, f1_micro=%f, precision_micro=%f, recall_micro=%f, NMI=%f, ADJ_RAND_SCORE=%f' % (
                a, b, c, acc, f1_macro, precision_macro, recall_macro, f1_micro, precision_micro, recall_micro, nmi,
                adjscore))
        fh.write('\r\n')
        fh.flush()
        fh.close()
    ...
    def evaluationClusterModelFromLabel_simple(self):
        nmi = metrics.normalized_mutual_info_score(self.true_label, self.pred_label)
        ari = metrics.adjusted_rand_score(self.true_label, self.pred_label)
        # f1 = metrics.f1_score(self.true_label, self.pred_label, average='macro')
